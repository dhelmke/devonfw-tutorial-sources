...
2. Install the _crypto-random-string_ and _lodash_ packages through ```yarn add <package_name>``` and import them.
...
So the handler that will register the user to the queue will be able to take the visitor information, generata a unique code with the _crypto-random-string_ package, insert it into our data base, along with the generated code and return that code with the hour to the visit, so the resulting handler will look like this:
