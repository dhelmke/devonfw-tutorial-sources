...
But before write the logic of our handler, as we need to generate a string code, to perform that, we gona add a function to the _utils.ts_ file at the root of our _handlers_ folder, and export it, so that function will return a alpha-numeric string code based on passed length:
