...
We have the `/jumpthequeue-core/src/main/resources/db/type/h2/V0001_Create_Sequence.sql, `V0002_Create_RevInfo.sql` and `V0003__Create_BinaryObject.sql` this table are used internally by default so don't delete it. We also have `/jumpthequeue-core/src/main/resources/db/migration/1.0/V0004_Add_blob_data.sql`. You can create your own schema table that your required. For e.g. `V0005_Create_Visitor.sql`
