...
Going back to our example application, link:JumpTheQueueDesign[Jump the Queue], we need to provide two basic functionalities: register a user (returning an access code) and show the registered queue members. To accomplish that we are going to work over two entities: _Visitor_ and _AccessCode_.

image::images/oasp4j/4.Components/visitor-accesscode.png[width="250", link="images/oasp4j/4.Components/visitor-accesscode.png"]

The _Visitor_ that will be defined with: _name_, _email_ and _phone_.

And the _Access Code_ that will represent a _code_ and the _date_.
...
INSERT INTO AccessCode (id, modificationCounter, code, dateAndTime, idVisitor) VALUES (1, 1, 'A01','2018-08-01', 1);
INSERT INTO AccessCode (id, modificationCounter, code, dateAndTime, idVisitor) VALUES (2, 1, 'A02','2018-08-02', 2);
----
...
[NOTE]
====
*Cobigen Health Check*: The first time we use Cobigen is recommended to check the health of the tool. To do so, right-click over an entity and select _Health Check_
...
Now we have to choose which packages we want to generate with the tool. The options are:
...
* _CRUD logic (all in one)_: is same as previous option but with the implementation of the logic layer in only one class instead of different use-cases classes.
...
To generate all the needed functionalities of our component we are going to select the following packages to be generated
...
Once the process has finished you may see that we need to also adjust manually some imports related to _Date_ type in:
...
image::images/oasp4j/4.Components/cobigen6_manual_import.png[link="images/oasp4j/4.Components/cobigen6_manual_import.png"]
