...
`V0003_Create__BinaryObject.sql` this table are already used by default so don't delete it. We therefore also have `/jumpthequeue-core/src/main/resources/db/migration/1.0/V0004__Add_blob_data.sql`. You can create your own schema table that your required. For e.g. `V0005__Create_Visitor.sql`
...
Finally we can provide a certain amount of mock data to start our app. Add a new sql script `/jumpthequeue-core/src/main/resources/db/migration/1.0/V0005__Master_data.sql` adding sample data:
