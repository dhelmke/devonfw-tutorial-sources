...
Now we can use layouts lets use it on _app.component.html_ to make it look like this:
...
image::images/oasp4js/3.BuildYourOwn/root_header.JPG[, link="images/oasp4js/3.BuildYourOwn/root_header.JPG"]
...
First we need to create a component to show as default, that will be our login view, later on we will modify it on it's section of this tutorial, but for now we just need to have it: stop the `ng serve` and run `ng generate component login`. It will add a folder to out project with all the files needed for a component. Now we can move on to the router task again. Run `ng serve` again to continue the development.

Let's create the file when the Router check for routes to route between components.
...
const routes: Routes = [  // Routes string, where Router will check the navigation and its properties.
  { path: 'login', component: LoginComponent},               // Redirect if url path is /login.
...
image::images/oasp4js/3.BuildYourOwn/root_router.JPG[, link="images/oasp4js/3.BuildYourOwn/root_router.JPG"]

As you can see, now what is in the body is the html of *LoginComponent*, this is because we told the Router to redirect to Login when the path is /Login, but also, redirect to Login as default if any of the other routes match with the path introduced.

We will definitely going to modify the header code to add logout and turn back options, but for the moment this is all regarding Root Component.
