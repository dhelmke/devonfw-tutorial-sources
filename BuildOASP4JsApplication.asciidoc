...
Another option to get this basic project structure with all its dependencies ans styles already set is to clone https://github.com/oasp/oasp4js-ng-project-seed/tree/develop-covalent[ng-project-seed] in the develop-covalent branch of the respository.

One you have cloned it, move to the project root folder and run a `yarn`to install all dependencies from package.json. The project also comes with some common functionalities.
