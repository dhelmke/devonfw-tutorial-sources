...
1. Angular Material and Covalent need the following modules to work: `CdkTableModule`, `BrowserAnimationsModule` and *every Covalent and Material Module* used in the application. So make sure you import them in the _imports array_ inside of _app.module.ts_. These modules come from `@angular/material`, `@angular/cdk`, `@angular/platform-browser/animations` and `@covalent/core`.
...
Remember that we need to import in _app.module_ every component of *Angular Material* and *Covalent Teradata* we use, for layouts it is CovalentLayoutModule:
