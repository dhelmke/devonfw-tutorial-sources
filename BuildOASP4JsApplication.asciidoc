...
This will add a grey background to the view and a card on top of it with the title: "Access", now that we have the basic structure of the view, lets add the form with the information to access to our queue number:
...
This forms contains three input container from Material and inside of them, the input with the properties listed above and making all required.
...
Let's move on to _code-viewer_ to made the template of the component. We need a big code number in the middle and a button to move to the queue:
...
Last thing to do regarding security is to make functional our log-off icon button in the header, we move to _app.component.html_ and add the correspondent (click) event calling for a function, in my case, called "logoff()".
...
At this point we are going to assume you have finished the OASP4J JumpTheQueue tutorial or, at least, you have downloaded the project and *have it running locally at localhost:8081*.
...
First, some configurations and modifications must be done to synchronize with how server works:
...
Once done all the preparations, lets move to _acces.service.ts_, here we had a simple `if` to check if the user inputs are what we expected, now we are going to call the server and it will manage all this logic to finally return us the information we need.
...
As we can see, and mentioned before, our preparations to this server calls we have done previously let us avoid changing anything in access component or template, everything should be working only doing that changes.
...
About the columns, the server sends us the data array composed by two objects: _visitor_ with the queue member information and _code_ with all the code information. As we are using the name of the queuer, the time it is expected to enter and its code, the column code should be like this:
...
Also, to make easier to the user read what is his position, Covalent Teradata provides with a functionality to check columns and modify the value shown, we used that to make bold the name of the user which corresponds with the user who is registered at the moment.
